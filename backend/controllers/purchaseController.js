...
function userFolderName(user) {
  return String(user?.username || user?.email || user?._id || "unknown-user")
    .trim()
    .replace(/[<>:"/\\|?*\x00-\x1f]+/g, "-")
    .replace(/\s+/g, " ")
    .replace(/^-+|-+$/g, "")
    .slice(0, 80) || "unknown-user";
}

function receiptFolders(type, recordDate, paidBy, userFolder) {
  const year = recordDate.getFullYear().toString();
  const month = recordDate.toLocaleString("en-US", { month: "long" });
  const folders = [sanitizeSegment(userFolder), type.toLowerCase(), year, sanitizeSegment(month)];
...
async function uploadReceiptToLocal(req, file, type, recordDate, paidBy, title, userFolder) {
  const folders = receiptFolders(type, recordDate, paidBy, userFolder);
...
exports.uploadToDrive = async (req, res) => {
  try {
    ...
    const receiptUserFolder = userFolderName(user);
    let uploaded;
    ...
    if (driveConnected) {
      try {
        ...
        uploaded = await uploadReceiptToLocal(req, uploadFile, type, recordDate, paidBy, title, receiptUserFolder);
      }
    } else {
      uploaded = await uploadReceiptToLocal(req, uploadFile, type, recordDate, paidBy, title, receiptUserFolder);
    }
