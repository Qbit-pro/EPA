...
export class ExpensePage implements OnDestroy {
  ...
  async confirmDelete(record: ExpenseRecord): Promise<void> {
  ...
  private deleteRecord(id: string): void {
    ...
    this.api.deleteExpense(id, token).pipe(
      finalize(() => {
        this.deletingId = '';
      })
    ).subscribe({
      next: () => {
        ...
        void this.presentToast('Expense deleted.', 'success');
      },
      error: error => {
        const message = error?.error?.message || error?.error?.error || 'Could not delete expense.';
        void this.presentToast(message, 'danger');
      }
    });
  }
